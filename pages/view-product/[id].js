...
const prod = productsJSON();

export default function ViewProd({ tmp : currProd }) {
  ...
  return (
    <>
    <Head>
    <title>{`HENCHGAL Construction | ${currProd.title}`}</title>
        <meta
          name="description"
    ...
    <nav
        ...
        <>
            ...
            <div className="prod-view">
              <div className="prod-img">
                <Swiper
                  ...
                  className="sld"
                >
                  {currProd.pics.map((d, i) => {
                      return (
                        <SwiperSlide key={i}>
                          <img key={i} src={`/images/prods/${d}`}></img>
                        </SwiperSlide>
                      );
                    })}
                </Swiper>
                  ...
                  <div>
                    ...
                    <p>
                      {currProd.description ||
                        "No description. Please contact us for any information."}
                    </p>
                  </div>
                ) : currProd.type === "gd" ? (
                  <>
                    ...
                    <div className="tabulated">
                      <div className="scr">
                        {[currProd].slice(0, 1).map((d, i) => {
                          let largest = 0;
                          d.inf.forEach((d) => {
                            largest =
                              Object.values(d)[0].length >= largest
                                ? Object.values(d)[0].length
                                : largest;
                          });
                          return (
                            <div
                              className="prod-table-inf"
                              key={i}
                              style={{
                                display: "grid",
                            ...
                            >
                              {d.inf.map((dx, ix) => {
                                const objkey = Object.keys(dx)[0];
                                const currentObjVals = Object.values(dx)[0];
                                return (
                                  <>
                                    <div
                                      style={{ fontWeight: "bold" }}
                                      className={`${
                                        ix % 2 === 0 ? "cell odd" : "cell"
                                      }`}
                                    >
                                      {objkey}
                                    </div>
                                    {currentObjVals.length === 0
                                      ? "-"
                                      : currentObjVals.map((dxx, ixx) => {
                                          return (
                                            <div
                                            key={ixx}
                                              className={`${
                                    ...
                                    {Array.from(
                                      {
                                        length: largest - currentObjVals.length,
                                      },
                                      (x, iex) => {
                                        return (
                                          <div
          ...
          <section className="other-prods">
            <div className="container">
              <div className="wrapper">
                <h1>Other products you might be interested in</h1>
                ...
                >
                   {productsJSON().map((d, i) => {

                    return (
                        <SwiperSlide key={i}>
                      <Link href={`/view-product/${d.uid}`}>
        ...
        <section className="contact-us">
          <div className="container">
           <div className="wrapper">
            <h1>Let's build something <span>bright</span>.</h1>
            <a href="/contact"><button>Contact Us <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6">
...
export async function getStaticPaths() {
    const paths = prod.map((d, i) => ({
        params: { id: d.uid.toString() }
    }))
    return { paths, fallback: false }
}

export async function getStaticProps({params}) {
    const tmp = productsJSON().find((obj) => {
        return obj["uid"] === parseInt(params.id);
    });
